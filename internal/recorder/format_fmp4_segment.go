...
func (s *formatFMP4Segment) initialize() {
	...
	if !s.startNTP.IsZero() && s.f.ri.rec.SegmentRoundDuration > 0 {
		s.endAfterNTP = s.startNTP.Round(s.f.ri.rec.SegmentRoundDuration)
		if s.startNTP.Sub(s.endAfterNTP) >= 0 {
			s.endAfterNTP = s.endAfterNTP.Add(s.f.ri.rec.SegmentRoundDuration)
		}
	}
}
